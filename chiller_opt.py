...
def cooling_demand(system_data,mode):
    if mode =='冷机模式':
        ...
        return 冷冻水进水流量 * 水比热熔 * (冷机冷冻水进水温度 - 冷机冷冻水出水温度)
        ...
        冷冻水进水流量=float(system_data[24])
        水比热熔=4.186*1000/3600
        return (板换冷冻水进水温度-冷机冷冻水出水温度)*冷冻水进水流量*水比热熔
        ...
        冷冻水进水流量=float(system_data[24])
        水比热熔=4.186*1000/3600
        return (板换冷冻水进水温度-板换冷冻水出水温度)*冷冻水进水流量*水比热熔
    ...
    for system_id in range(1, num_systems + 1):
        if is_system_running(latest_time_rows,system_id):
            ...
            while system_data is None:
                ...
                cooling_demand+=cooling_demand(system_data,mode)
...
#计算 reward
def calculate_reward_from_power_consumption(current_state,current_cooling_demand,next_state,next_cooling_demand):
    ...
    for outlet_idx, inlet_idx, flow_idx,chill_power_idx in zip(chiller_outlet_temp_indices, chiller_inlet_temp_indices, chill_water_flow_indices,chiller_power):
        ...
        # Optionally, handle cases where the chiller was off and remains off or turns on
            ...
            if float(next_state[outlet_idx])>13:
    ...
    reward=w1*consumption_reduced+turn_off_reward+cool_efficiency_reward+ COP_reward - w3 * Deviation_from_Set_Points - w4*Operational_Extremes
    return reward
...
#         Calculate reward based on the power consumption difference
        reward = calculate_reward_from_power_consumption(current_state,current_cooling_demand,next_state,next_cooling_demand)
#         ...
